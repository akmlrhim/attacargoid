...
class InfiniteGridMenu {
    ...
    #animate(deltaTime) {
        ...
        this.instancePositions.forEach((p, ndx) => {
            const transformed = vec3.transformQuat(vec3.create(), p, this.control.orientation);
            const s = (Math.abs(transformed[2]) / this.SPHERE_RADIUS) * SCALE_INTENSITY + (1 - SCALE_INTENSITY);
            const finalScale = s * scale;
            const matrix = mat4.create();
            mat4.multiply(matrix, matrix, mat4.fromTranslation(mat4.create(), vec3.negate(vec3.create(), transformed)));
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
    ...
    const handleButtonClick = () => {
        ...
        if (activeItem.link.startsWith('http')) {
            ...
            document.querySelector(activeItem.link)?.scrollIntoView({ behavior: 'smooth' });
        }
    };
    ...
    return (
        <div className="infinite-menu-root">
            ...
            {activeItem && (
                <>
                    <div className={`infinite-menu-info ${isMoving ? 'inactive' : 'active'}`}>
                        <div className="infinite-menu-tag">{activeItem.tag}</div>
                        <h3 className="infinite-menu-title">{activeItem.title}</h3>
                        <p className="infinite-menu-desc">{activeItem.description}</p>
                    </div>

                    <button
                        onClick={handleButtonClick}
                        className={`infinite-menu-btn ${isMoving ? 'inactive' : 'active'}`}
                        aria-label={`Lihat ${activeItem.title}`}
                    >
                        <span>&#x2197;</span>
                    </button>
                </>
