        ...
        <div className="sr max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-4 sm:mb-5 p-3.5 sm:p-4 bg-navy/5 rounded-2xl border border-navy/10">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-black">{region.name}</h3>
              <p className="text-black text-xs">
                {region.cities.length} kota terjangkau
              </p>
            </div>
            {activeRegion === "kalsel" && (
            ...
            {region.cities.map((city) => {
              ...
              if (isEtc) {
                return (
                  <div
                    key={city}
                    className="flex items-center gap-2 px-2.5 sm:px-3 py-2 sm:py-2.5 rounded-xl border border-dashed border-gray-200 col-span-2 sm:col-span-1"
                  >
                    <span className="text-xs sm:text-sm text-black italic">
                      {city}
                    </span>
                  </div>
                );
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-xs sm:text-sm font-semibold truncate ${isHub ? "text-white" : "text-black"}`}
                  >
                    {city}
                  </span>
                  {isHub && (
                    <span className="ml-auto shrink-0 text-[9px] sm:text-[10px] bg-white/25 text-white px-1 sm:px-1.5 py-0.5 rounded font-bold">
                      HUB
                    </span>
                  )}
                </div>
          ...
          <p className="text-center text-black italic text-sm mt-8">
